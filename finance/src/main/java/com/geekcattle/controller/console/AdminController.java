...
public class AdminController {
    ...
    @RequestMapping(value = "/viewpwd", method = {RequestMethod.GET})
    public String editpwd(Model model) {

           Admin admin =adminService.getById(AdminShiroUtil.getUserInfo().getUid()) ;

          model.addAttribute("admin",admin);

          return "console/admin/editpwd";

    }

    @RequiresPermissions("admin:editpwd")
    @RequestMapping(value = "/viewpwd", method = {RequestMethod.POST})
    public String savepwd(Model model, String password) {


        String uid = AdminShiroUtil.getUserInfo().getUid();
        if (StringUtils.isNotEmpty(uid) && StringUtils.isNotEmpty(password)) {
            ...
            if (!"null".equals(admin)) {
                ...
                pwdAdmin.setUid(uid);
                adminService.updatePwd(pwdAdmin);
    ...
    @ResponseBody
    public ModelMap savePwd(String uid, String password) {
        try {
            if (StringUtils.isNotEmpty(uid) && StringUtils.isNotEmpty(password)) {
                Admin admin = adminService.getById(uid);
                if (!"null".equals(admin)) {
